...
async fn decode_transactions<Pool: TransactionPool>(
    ...
    let computed_hash = keccak256(&block_data);

    // if computed_hash != block_data_hash {
    //     eyre::bail!("block data hash mismatch")
    // }

    // Get encrypted transactions either from the blobs or directly from the block data
    let encrypted_transactions = if matches!(tx.tx_type(), TxType::Eip4844) {
        // Handle EIP-4844 transactions with encrypted blobs
        let blobs_with_commitments: Vec<_> = if let Some(sidecar) = pool.get_blob(tx.hash)? {
        ...
        let blobs = blobs_with_commitments
            .into_iter()
        ...
        // Decode blobs and concatenate them to get the encrypted transactions
        let encrypted_data = SimpleCoder::default()
            .decode_all(&blobs)
            .ok_or(eyre::eyre!("failed to decode blobs"))?
            .concat();

        // Parse the encrypted data as a list of encrypted transactions
        Vec::<Bytes>::decode(&mut encrypted_data.as_ref())?
    } else {
        // For non-EIP-4844 transactions
        // Parse block_data as a list of encrypted transactions
        Vec::<Bytes>::decode(&mut block_data.as_ref())?
    };

    // Decrypt and decode each transaction
    let transactions = encrypted_transactions
        .into_iter()
        .map(|encrypted_tx| {
            // Decrypt the transaction
            let decrypted_tx_data = decrypt::decrypt_tx(
                encrypted_tx.to_vec().as_slice(),
                dec_key.as_slice().try_into().unwrap(),
            );
            // Decode the decrypted transaction
            let tx = TransactionSigned::decode(&mut decrypted_tx_data.as_ref())?;
            // Recover the signer
            let sender = tx.recover_signer().ok_or(eyre::eyre!("failed to recover signer"))?;
            // Ensure the transaction has the correct chain ID
            if tx.chain_id() != Some(CHAIN_ID) {
                eyre::bail!("Invalid chain ID in transaction")
            }
            Ok((tx, sender))
        })
